const User = require('../models/user-model')
 ...
 const login = async(req,res)=>{
  ...
  user.login=true
  return  res.status(200).json({userId:user.id,message:'ok',isAuth:user.login})}
catch(e){
...
res.status(400).json({message:'error'})
 ...
 const register = async(req,res)=>{
     try{
        ...
        await user.save()
        return res.status(200).json({message:'Пользователь создан',isAuth:user.login,userId:user.id})
